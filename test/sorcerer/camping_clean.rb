...
class Object
  ...
  def meta_def m, &b
    (class << self
      self
    end).send :define_method, m, &b
  end
end

module Camping
  C = self
  S = nil rescue IO.read(__FILE__)
  ...
  module Base
    attr_accessor :env, :request, :root, :input, :cookies, :state, :status,
      :headers, :body
    T = {}
    L = :layout
    ...
    def n h
      Hash === h ? h.inject(H[]) { |m, ((k), (v))| m[k] = n(v)
      m } : h
    end
